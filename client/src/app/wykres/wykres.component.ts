...
const httpOptions = { //przekazywana z zadaniem w formacie Bearer {Token}
  headers: new HttpHeaders({
    Authorization: 'Bearer ' + JSON.parse(localStorage.getItem('user'))?.token,
  }),
};
@Injectable({ providedIn: 'root' }) //hmmm
@Component({
  selector: 'app-wykres',
...
export class WykresComponent implements OnInit {
  ...
  toJson() {
    return this.http
      .get('https://localhost:5001/api/db/saveJson', httpOptions)//przekazywany jest token z zadaniem
      .subscribe(
        (res) => {
